...
for i in range(rv.num_samples):
     ...
     velo = [rv.W_D_V100_samples[i],rv.W_D_V1_samples[i]]
     #print(Tes_velo)
     
     bounds =[]
     for j in range(len(rv.W_D_nodes)-1):
         #print(j)
         a = rv.W_D_nodes[j]/(velo[j]*rv.dt*(1/1000))
         b= rv.W_D_nodes[j+1]/(velo[j]*rv.dt*(1/1000))
         bounds.append(a)
         bounds.append(b)
     #print(Tes_bounds) 
...
#print(df)
df.to_csv('output/white+donjec/White+Donjec_River_Mouth_Values.csv')
df.to_csv('output/analysis/White+Donjec_River_Mouth_Values.csv')
