...
for i in range(rv.num_samples):
     ...
     velo = [rv.Por_V100_samples[i],rv.Por_V1_samples[i]]
     #print(Por_velo)
     
     bounds =[]
     for j in range(len(rv.Por_nodes)-1):
         #print(j)
         a = rv.Por_nodes[j]/(velo[j]*rv.dt*(1/1000))
         b= rv.Por_nodes[j+1]/(velo[j]*rv.dt*(1/1000))
         bounds.append(a)
         bounds.append(b)
     #print(Por_bounds) 
     #print(len(Por_bounds))
    ...
    #values for the while loop
     ...
     #Por_imax values = the last item of the Por_i_sum
     imax.append(i_sum[len(i_sum)-1])
     #print(imax)
     
                                                #   time_calculation    #
     #print("Por_imax_values:",Por_imax[i])
...
#print(df)
df.to_csv('output/porcupine/Porcupine_River_Mouth_Values.csv')
df.to_csv('output/analysis/Porcupine_River_Mouth_Values.csv')
