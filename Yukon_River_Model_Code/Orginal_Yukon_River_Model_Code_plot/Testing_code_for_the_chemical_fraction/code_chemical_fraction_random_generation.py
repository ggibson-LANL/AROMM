...
def adjust_samples_to_sum(samples, target_sum=1):
    ...
    #print(samples.shape)
    #Sample's dimensions were assinged to obtain values for the num samples and num parameters
    ...
    #sum = samples[:,0]+samples[:,1]+samples[:,2]+samples[:,3]+samples[:,4]+samples[:,5]+samples[:,6]+samples[:,7]
    #print(sum)
    #print(sum == 1.000,"**")    
    ...
    while not np.all(success):
        for i in range(num_samples):
            ...
            param_bounds = [(bounds[j][0], bounds[j][1]) for j in range(num_parameters)]
