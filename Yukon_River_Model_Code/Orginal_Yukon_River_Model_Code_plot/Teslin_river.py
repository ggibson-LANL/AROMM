...
Prod1 = []        #Polypeptide
Prod2 = []        #Amino acid
Prod3 = []        #Heteropolycondensate
Prod4 = []        #Polysaccharide
Prod5 = []        #Oligosaccharide
Prod6 = []        #Humic acid
Prod7 = []        #Monosaccharides
Prod8 = []        #Inorganic carbon
Prod9 = []        #Lipids
Prod10 = []       #Pigments
Prod11 = []       #Porphyrin
Prod12 = []       #CDOM
...
for i in range(rv.num_samples):
    ...
    #values for the while loop
     ...
     #Inserting end values at river mouth lists for all chemical
     Prod0.append(Prod[j-1,0])
     Prod1.append(Prod[j-1,1])
     Prod2.append(Prod[j-1,2])
     Prod3.append(Prod[j-1,3])
     Prod4.append(Prod[j-1,4])
     Prod5.append(Prod[j-1,5])
     Prod6.append(Prod[j-1,6])
     Prod7.append(Prod[j-1,7])
     Prod8.append(Prod[j-1,8])
     Prod9.append(Prod[j-1,9])
     Prod10.append(Prod[j-1,10])
     Prod11.append(Prod[j-1,11])
     #Prod12.append(Prod[j-1,12]) # no need as the concentrations are coming as a sum
     Prod13.append(Prod[j-1,13])
     Prod14.append(Prod[j-1,14])
     Prod15.append(Prod[j-1,15])
     #Prod16.append(Prod[j-1,16]) # no need as the concentrations are coming as a sum
     #Prod17.append(Prod[j-1,17]) # no need as the concentrations are coming as a sum
...
df.to_csv('output/analysis/Teslin_River_Mouth_Values.csv')
